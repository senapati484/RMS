// scripts/delete-200-random.ts — Run with: npx tsx scripts/delete-200-random.ts
...
async function deleteRandom200() {
  ...
  const deleteCount = Math.min(200, totalBefore)

  // Use MongoDB $sample aggregation to select 200 random products
...
deleteRandom200().catch((err) => {
  console.error('❌ Error during 200 random product deletion:', err)
  process.exit(1)
})
