...
export default function NewOrderPage() {
  ...
  const addToCart = (product: Product) => {
    ...
    if (existing) {
      ...
      const baseRate = 500 // default ₹500/day base rate — should come from product pricing
  ...
  return (
    <div className="space-y-6 max-w-5xl mx-auto">
      <div>
        <h1 className="text-white text-2xl font-bold">New Order</h1>
        <p className="text-white/40 text-sm mt-1">Create a rental order</p>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 space-y-4">
              ...
              {products.map(p => (
                <div key={p._id} className="flex items-center gap-3 bg-white/5 rounded-xl p-3 hover:bg-white/10 transition-colors">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="text-white/30 text-xs">Stock: {p.availableStock}</div>
                  </div>
                  <button
                    onClick={() => addToCart(p)}
                    disabled={p.availableStock === 0}
                    className="w-7 h-7 bg-[#F26522] hover:bg-[#e05510] disabled:opacity-30 rounded-lg flex items-center justify-center text-white transition-colors"
                  >
                    <Plus size={12} />
                  </button>
                </div>
          ...
          {/* Rental period */}
          <div className="liquid-glass border border-white/10 rounded-2xl p-5">
            <h2 className="text-white/60 text-xs font-medium uppercase tracking-wider mb-4">Rental Period</h2>
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                <label className="block text-white/40 text-xs mb-2">Start Date</label>
                <input
                  type="date"
                  value={rentalStart}
              ...
              <div>
                <label className="block text-white/40 text-xs mb-2">End Date (auto)</label>
                <input
                  type="date"
                  value={rentalEnd}
                  readOnly
        ...
        <div className="space-y-4">
          <form onSubmit={handleSubmit} className="liquid-glass border border-white/10 rounded-2xl p-5 space-y-4">
            <h2 className="text-white/60 text-xs font-medium uppercase tracking-wider">Order Summary</h2>
            ...
            <div>
              <label className="block text-white/40 text-xs mb-2">Delivery</label>
